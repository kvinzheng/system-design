...
function Toast({ toast: t, onDismiss }) {
  ...
  return h('div', {
    ...
    h('span', { 'aria-hidden': true, style: { fontSize: 16 } }, color.icon),
    h('div', { style: { flex: 1, minWidth: 0 } },
      h('div', { style: { fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 } },
        h('span', { style: { flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, t.message || t.title || ''),
        count > 1
          ? h('span', { style: { background: '#111827', color: '#fff', fontSize: 10, padding: '1px 6px', borderRadius: 10, fontWeight: 700 } }, `\u00d7${count}`)
          : null,
      ),
      t.body
        ? h('div', { style: { fontSize: 12, color: '#374151', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, t.body)
        : null,
    ),
    h('button', {
...
export default function Toaster() {
  ...
  return h(React.Fragment, null,
    h('style', null, `@keyframes pn_toastIn {
    ...
    h('div', {
      ...
      toasts.map((t) => h(Toast, { key: t.id, toast: t, onDismiss: toastStore.remove })),
    ),
  );
}
