...
import { getPrefs, setPrefs, subscribePrefs } from '../core/prefs';
    ...
    <>
      ...
      <div style={styles.popover} role="dialog" aria-label="Notification preferences">
        ...
        <Section label="Density">
          <select value={prefs.density} onChange={(e) => update({ density: e.target.value })} style={styles.select}>
        ...
        <Section label="Focus mode">
          <label style={styles.row}>
            <input type="checkbox" checked={prefs.focusMode} onChange={(e) => update({ focusMode: e.target.checked })} />
        ...
        <Section label="Quiet hours">
          ...
          {prefs.quietHours && (
            <div style={styles.timeRow}>
              <span>From</span>
              <select value={prefs.quietHours.start} onChange={(e) => update({ quietHours: { ...prefs.quietHours, start: Number(e.target.value) } })} style={styles.select}>
                {Array.from({ length: 24 }, (_, i) => <option key={i} value={i}>{String(i).padStart(2, '0')}:00</option>)}
              </select>
              <span>to</span>
              <select value={prefs.quietHours.end} onChange={(e) => update({ quietHours: { ...prefs.quietHours, end: Number(e.target.value) } })} style={styles.select}>
                {Array.from({ length: 24 }, (_, i) => <option key={i} value={i}>{String(i).padStart(2, '0')}:00</option>)}
              </select>
            </div>
          )}
